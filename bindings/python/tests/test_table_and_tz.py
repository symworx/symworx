...
from symworx.core import io, math


def test_whitespace_explicit_names(tmp_path: Path):
    p = tmp_path / "rr.txt"
    p.write_text("0.000 1.304\n1.304 1.304\n", encoding="utf-8")
    t = io.load_numeric_table(str(p), delimiter="whitespace", has_headers=False, names=["t_s", "rr_s"])
...
def test_headered_csv_skips_non_numeric(tmp_path: Path):
    ...
    t = io.load_numeric_table(str(p))
    assert t.headers == ["x", "y"]
    assert t.n_rows() == 2


def test_us_eastern_august_is_edt():
    tz = math.TimeZone.parse("US/Eastern")
    assert tz.offset_hours(2017, 8, 14, 8, 20, 0) == -4
    assert math.TimeZone.parse("EST").offset_hours(2017, 8, 14, 8, 20, 0) == -5
