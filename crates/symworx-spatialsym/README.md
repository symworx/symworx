...
- `PlayingDimensions` (rectangular field size in meters).
...
```rust
use symworx_spatialsym::{Point2, Vec2, pairwise_distances, derive_velocities_from_times, SpaceAction, PlayingDimensions};

let pts = vec![Point2::new(0.,0.), Point2::new(1.,0.)];
let dists = pairwise_distances(&pts);
let vels = derive_velocities(&pts, 0.1); // dt = 0.1s

let dims = PlayingDimensions::new(105.0, 68.0); // meters, generic
