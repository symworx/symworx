...
See `crates/symworx-loadsym-db/sql/schema.sql` in the symworx repo (or run `cargo run -p symworx-loadsym --features db -- db print-schema`).
...
2. `export SYMLOAD_USER="nberry.fitdata@gmail.com"`
...
See the examples in `crates/symworx-loadsym/README.md` and `docs/loadsym-personal-starter.md`.
...
3. `psql` the schema.
4. Drop a real .fit in inbox, run ingest.
...
Questions or want us to generate more of the starter files? Just say.
